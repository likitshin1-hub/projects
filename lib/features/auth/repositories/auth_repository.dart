...
class AuthRepository {
  ...
  Future<ApiResult<String>> login({
    ...
    try {
      ...
      if (token.isEmpty) {
        return ApiResult.failure('ไม่พบ access token จากเซิร์ฟเวอร์');
      }
  ...
  Future<ApiResult<String>> loginWithFacebook({
    ...
    if (_useMock) {
      ...
      await SecureStorage.saveAccessToken('mock_token_facebook');
  ...
  Future<ApiResult<String>> loginWithLine({required String accessToken}) async {
    if (_useMock) {
      await Future.delayed(const Duration(milliseconds: 800));

      await SecureStorage.saveAccessToken('mock_token_line');
