...
class NetworkException extends AppException {
  const NetworkException({super.message = 'ไม่สามารถเชื่อมต่ออินเทอร์เน็ตได้'});
}

class TimeoutException extends AppException {
  const TimeoutException({super.message = 'การเชื่อมต่อหมดเวลา กรุณาลองใหม่'});
}

class CancelledException extends AppException {
  const CancelledException({super.message = 'คำขอถูกยกเลิก'});
}
...
class ValidationException extends AppException {
  ...
  String? firstErrorOf(String field) => errors?[field]?.firstOrNull;
...
class UnknownException extends AppException {
  const UnknownException({super.message = 'เกิดข้อผิดพลาดที่ไม่ทราบสาเหตุ'});
}
  ...
  switch (e.type) {
    ...
    case DioExceptionType.badResponse:
      ...
      return switch (statusCode) {
        ...
        404 => NotFoundException(message: message ?? 'ไม่พบข้อมูลที่ต้องการ'),
